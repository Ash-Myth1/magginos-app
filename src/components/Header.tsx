  ...
  return (
    <header className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-slate-100 z-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3 cursor-pointer group" onClick={() => navigate('/')} role="link" aria-label="Go to homepage" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter') navigate('/'); }}>
          ...
          {currentView === 'customer' ? (
            currentUser ? (
              <>
                ...
                <button onClick={() => setShowCheckout(true)} aria-label="Shopping cart" className="relative p-2 text-slate-800 hover:bg-slate-100 rounded-xl transition-all">
                ...
                <button onClick={onLogout} aria-label="Sign out" className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                  <LogOut size={20} />
                </button>
              </>
            ) : (
            ...
            <button onClick={() => navigate('/')} aria-label="Exit kitchen admin view" className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-slate-800 transition-all active:scale-95">
